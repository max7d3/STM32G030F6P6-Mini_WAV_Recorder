...
#define FCLK_SLOW() {SPI2_SetSpeedLow();}	/* Set SCLK = slow, approx 280 KBits/s*/
#define FCLK_FAST() {SPI2_SetSpeedHigh();}	/* Set SCLK = fast, approx 4.5 MBits/s */

#define CS_HIGH()	SPI2_SD_CS_HIGH
#define CS_LOW()	SPI2_SD_CS_LOW
...
void SPI_Timer_On(uint32_t waitTicks) {
    spiTimerTickStart = TICK_Get();
    spiTimerTickDelay = waitTicks;
}

uint8_t SPI_Timer_Status() {
    return ((TICK_Get() - spiTimerTickStart) < spiTimerTickDelay);
}
...
)
{
	...
	waitSpiTimerTickStart = TICK_Get();
	waitSpiTimerTickDelay = (uint32_t)wt;
	do {
		d = xchg_spi(0xFF);
		/* This loop takes a time. Insert rot_rdq() here for multitask envilonment. */
	} while (d != 0xFF && ((TICK_Get() - waitSpiTimerTickStart) < waitSpiTimerTickDelay));	/* Wait for card goes ready or timeout */
